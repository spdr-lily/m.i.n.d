...
from app.services.crud_service import ReferenceService
...
async def list_sex_types(db: Session = Depends(get_db), _=Depends(require_permission(Permission.READ_REFERENCE))):
...
async def get_sex_type(sex_type_id: int, db: Session = Depends(get_db), _=Depends(require_permission(Permission.READ_REFERENCE))):
    service = ReferenceService(db)
...
async def create_sex_type(data: SexTypeResponse, db: Session = Depends(get_db), _=Depends(require_permission(Permission.WRITE_REFERENCE))):
    service = ReferenceService(db)
    try:
...
async def list_gender_identities(db: Session = Depends(get_db), _=Depends(require_permission(Permission.READ_REFERENCE))):
    service = ReferenceService(db)
...
async def get_gender_identity(gender_identity_id: int, db: Session = Depends(get_db), _=Depends(require_permission(Permission.READ_REFERENCE))):
    service = ReferenceService(db)
...
async def create_gender_identity(data: GenderIdentityResponse, db: Session = Depends(get_db), _=Depends(require_permission(Permission.WRITE_REFERENCE))):
    service = ReferenceService(db)
    try:
...
async def list_education_levels(db: Session = Depends(get_db), _=Depends(require_permission(Permission.READ_REFERENCE))):
    service = ReferenceService(db)
...
async def get_education_level(education_level_id: int, db: Session = Depends(get_db), _=Depends(require_permission(Permission.READ_REFERENCE))):
    service = ReferenceService(db)
...
async def create_education_level(data: EducationLevelResponse, db: Session = Depends(get_db), _=Depends(require_permission(Permission.WRITE_REFERENCE))):
    service = ReferenceService(db)
    try:
...
async def list_ethnicities(db: Session = Depends(get_db), _=Depends(require_permission(Permission.READ_REFERENCE))):
    service = ReferenceService(db)
    return [EthnicityResponse.model_validate(eth) for eth in service.list_ethnicities()]


@router.get("/ethnicities/{ethnicity_id}")
async def get_ethnicity(ethnicity_id: int, db: Session = Depends(get_db), _=Depends(require_permission(Permission.READ_REFERENCE))):
    service = ReferenceService(db)
...
async def create_ethnicity(data: EthnicityResponse, db: Session = Depends(get_db), _=Depends(require_permission(Permission.WRITE_REFERENCE))):
    service = ReferenceService(db)
    try:
